...
static uint8_t raw_bytes[256];
static sx127x_t sx127x;
static uint8_t WhiteningKeyLSB = 0xFF;
static uint8_t WhiteningKeyMSB = 0x00;
...
void SX1232RadioComputeWhitening(uint8_t *buffer, uint16_t bufferSize )
{
    ...
    // WhiteningKeyLSB is 0xFF at init
    for( j = 0; j < bufferSize - 1; j++ )
    {
        ...
        for( i = 0; i < 8; i++ )
        {
            ...
            WhiteningKeyLSB = ((((WhiteningKeyMSBPrevious << 7 ) & 0x80) |
                              ((WhiteningKeyLSB >> 1 ) & 0xFF)));
        }
        revertedWhiteningKeyLSB = (WhiteningKeyLSB & 0xF0) >> 4 |
                                  (WhiteningKeyLSB & 0x0F) << 4;
        revertedWhiteningKeyLSB = (revertedWhiteningKeyLSB & 0xCC) >> 2 |
                                  (revertedWhiteningKeyLSB & 0x33) << 2;
        revertedWhiteningKeyLSB = (revertedWhiteningKeyLSB & 0xAA) >> 1 |
                                  (revertedWhiteningKeyLSB & 0x55) << 1;
    }
}
        ...
        switch (event) {
            case NETDEV_EVENT_RX_COMPLETE:
                ...
                if (sx127x.settings.modem == SX127X_MODEM_LORA) {
                    ...
                    //SX1232RadioComputeWhitening(raw_bytes, len);
                    printf("{Payload: ");
